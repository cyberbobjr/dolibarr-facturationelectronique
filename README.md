...
2. **Factures Émises (Ventes)** :
   ...
   - **Exonérations de TVA (mentions VATEX)** : pour les factures comportant des lignes sans TVA (export hors UE, livraison intracommunautaire, autoliquidation, franchise en base, opérations hors champ…), le module attribue automatiquement le **code de motif d'exonération VATEX** (BT-121) et le **texte légal** (BT-120) attendus par la norme EN16931, évitant le rejet `ExemptionReasonCode is not allowed`. Le mapping par catégorie est intégré (`AE`→VATEX-EU-AE, `G`→VATEX-EU-G, `K`→VATEX-EU-IC, `O`→VATEX-EU-O, `E`→VATEX-FR-FRANCHISE) et peut être surchargé par facture ou par tiers via une **liste déroulante** des codes VATEX officiels (avec descriptions). L'onglet *Facturation Électronique* de la facture affiche un récapitulatif des mentions qui seront transmises.
...
3. **Factures Reçues (Achats)** :
   ...
   - Liste des factures reçues rafraîchie en direct depuis la plateforme, avec un bouton **« Récupérer les nouvelles factures »** pour interroger le PDP à la demande.
   ...
   - **Mode « téléchargement seul »** (option *Autoriser l'import* dans la configuration) : lorsque l'import est désactivé, aucune facture fournisseur n'est créée automatiquement ; l'utilisateur peut uniquement **télécharger le PDF/XML** de chaque facture reçue. La tâche cron respecte également ce réglage.
