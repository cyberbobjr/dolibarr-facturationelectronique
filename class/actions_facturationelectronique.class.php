	...
	public function addMoreActionsButtons($parameters, &$object, &$action, $hookmanager)
	{
		...
		if ($parameters['currentcontext'] === 'thirdpartycard' || $parameters['currentcontext'] === 'invoicecard') {
			...
			if ($feat_siren && $parameters['currentcontext'] === 'thirdpartycard') {
				...
				// The lookup source is now chosen inside the modal, so the button no longer
				// advertises a single provider.
				$btn_text = empty($siren) ? "Rechercher SIREN" : "Vérifier/Mettre à jour SIREN";
